...
#define TOPIC       "MCHP/pubTest"
#define QOS         1
#define TIMEOUT     10000L
volatile MQTTClient_deliveryToken deliveredtoken;
unsigned char mqttRevBuf[8];
volatile bool mqttRevValided = false;

static int GetDateTime(char * psDateTime)
{
    time_t nSeconds;
    struct tm * pTM;
...
// mqtt process
	auto threadMQTT = [](MOTO_STRUCT& val) {
		MQTTClient client;
	    MQTTClient_connectOptions conn_opts = MQTTClient_connectOptions_initializer;
	    int rc;
	    int ch;

		std::cout << "Message from MQTT Thread" << std::endl;
	    ...
	    if ((rc = MQTTClient_connect(client, &conn_opts)) != MQTTCLIENT_SUCCESS)
	    {
	    	...
	    	while(1) {
				...
				if( mqttRevValided ) {
					...
					val.turnRightlight = (LIGHT)mqttRevBuf[2];
					val.turnLeftlight = (LIGHT)mqttRevBuf[3];
	...
	std::thread tMqtt(threadMQTT, std::ref(motoValue));
       ...
       //needle move function implemented by lambda
		...
		//if (true)
        {
            ...
            else
            {
                low_pri_q.push([&]()
                {
					//std::cout << motoValue.headlight << std::endl;

					if( motoValue.headlight == LIGHT_OFF ) {
						farlightPtr->show();
					} else {
						farlightPtr->hide();
					}
            ...
            else
            {
                low_pri_q.push([&]()
                {
					...
					if( motoValue.turnLeftlight == LIGHT_OFF ) {
						left_blinkPtr->show();
					} else {
						left_blinkPtr->hide();
					}
